...
describe('roundBigNumToDecimalPlace delegates to roundToDecimals half-up', () => {
	...
	// Every entry is an intended fix: the old implementation routed the value
	// through `toNum()`, so it inherited the double's faults.
	const TIE = 'ties on negatives now go away from zero, not toward +Infinity';
	...
	const divergences: Record<string, Divergence> = {
		'-1.5 @0dp p6': { behaviour: TIE, old: '-1.000000', next: '-2.000000' },
		'-1.5 @0dp p9': {
			behaviour: TIE,
			old: '-1.000000000',
			next: '-2.000000000',
		},
		'-2.5 @0dp p6': { behaviour: TIE, old: '-2.000000', next: '-3.000000' },
		'-2.5 @0dp p9': {
			behaviour: TIE,
			old: '-2.000000000',
			next: '-3.000000000',
		},
		'-0.5 @0dp p6': { behaviour: TIE, old: '0.000000', next: '-1.000000' },
		'-0.5 @0dp p9': {
			behaviour: TIE,
			old: '0.000000000',
			next: '-1.000000000',
	...
	it(TIE, () => {
		...
		expect(roundBigNumToDecimalPlace(half, 0).print()).to.equal('-2.000000');
	});
